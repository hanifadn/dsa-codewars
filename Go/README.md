...
Solutions use `package kata` to match Codewars. There is no `func main` in these files; run tests on Codewars via the link in the header, or add a small `package main` in the same module that imports the kata package and calls its functions, or use a `_test.go` file with `go test`.
